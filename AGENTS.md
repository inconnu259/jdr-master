...
- Ne rien lancer sur l'hôte : il n'y a aucun `node_modules`, `docker-compose.yml` les monte dans des volumes Docker nommés. Aucun binaire local (`eslint`, `jest`, `vitest`, `tsc`, `prisma`) n'est exécutable — passer par `docker compose exec api|web pnpm <...>`, stack démarrée.
