...
function makeMockPrisma() {
  const mockCreate    = jest.fn(async ({ data }: { data: object }) => ({ id: 'new-' + Math.random(), ...data }));
  ...
  return { mockPrisma, mockCreate, mockUpdate, mockUpdateMany, mockFindUnique, mockFindMany };
}
...
describe('AvailabilityService.splitOccurrence', () => {
  ...
  beforeEach(() => {
    ...
    service = new AvailabilityService(mocks.mockPrisma as unknown as PrismaService);
  ...
  it('normal split : crée R1, Rmod, R2 et soft-delete R', async () => {
    ...
    const { created, deleted } = await service.splitOccurrence(DECL_ID, USER_ID, '2026-07-01', 'delete');
    ...
    expect(mockUpdate).toHaveBeenCalledWith({ where: { id: DECL_ID }, data: expect.objectContaining({ expiresAt: expect.any(Date) }) });
  ...
  it('right-edge (dPlus7 > expiresAt) : seulement R1 + Rmod, pas de R2', async () => {
    ...
    mockFindUnique.mockResolvedValue(makeRecurring({ startDate: WED1, endDate: null, expiresAt: expiresAfterWed2ButBeforeWed3 }));
    await service.splitOccurrence(DECL_ID, USER_ID, '2026-07-01', 'delete');
    expect(mockCreate).toHaveBeenCalledTimes(2);
    const calls = mockCreate.mock.calls as [{ data: Record<string, unknown> }][];
  ...
  it('action modify : Rmod utilise les valeurs dto', async () => {
    ...
    await service.splitOccurrence(DECL_ID, USER_ID, '2026-07-01', 'modify', { kind: 'AVAILABLE', slot: 'MORNING' });
    const calls = mockCreate.mock.calls as [{ data: Record<string, unknown> }][];
    const rmodCall = calls.find((c) => c[0].data.recurKind === 'PUNCTUAL');
    expect(rmodCall![0].data.kind).toBe('AVAILABLE');
...
function makePrismaDecl(overrides: Partial<{
...
describe('AvailabilityService.findConflictsForCreate', () => {
  ...
  it('même kind → pas de conflit', async () => {
    ...
    const dto = { kind: 'UNAVAILABLE' as const, recurKind: 'RECURRING' as const, dayOfWeek: 3, slot: 'EVENING' as DaySlot, expiresAt: FUTURE.toISOString() };
  ...
  it('slot différent (MORNING vs EVENING) → pas de conflit', async () => {
    ...
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'RECURRING' as const, dayOfWeek: 3, slot: 'EVENING' as DaySlot, expiresAt: FUTURE.toISOString() };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(0);
  });
  ...
  it('RECURRING vs RECURRING même dayOfWeek → conflit', async () => {
    mockFindMany.mockResolvedValue([makePrismaDecl({ kind: 'UNAVAILABLE', recurKind: 'RECURRING', dayOfWeek: 3 })]);
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'RECURRING' as const, dayOfWeek: 3, slot: 'EVENING' as DaySlot, expiresAt: FUTURE.toISOString() };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(1);
  });

  it('RECURRING vs RECURRING dayOfWeek différent → pas de conflit', async () => {
    mockFindMany.mockResolvedValue([makePrismaDecl({ kind: 'UNAVAILABLE', recurKind: 'RECURRING', dayOfWeek: 4 })]); // jeudi
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'RECURRING' as const, dayOfWeek: 3, slot: 'EVENING' as DaySlot, expiresAt: FUTURE.toISOString() };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(0);
  });
  ...
  it('nouveau RECURRING vs PUNCTUAL : le mercredi est dans la plage PUNCTUAL → conflit', async () => {
    ...
    mockFindMany.mockResolvedValue([makePrismaDecl({
      kind: 'UNAVAILABLE', recurKind: 'PUNCTUAL', dayOfWeek: null, startDate: WED1, endDate: WED3,
    })]);
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'RECURRING' as const, dayOfWeek: 3, slot: 'EVENING' as DaySlot, startDate: '2026-06-01', expiresAt: FUTURE.toISOString() };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(1);
  });
  ...
  it('nouveau RECURRING vs PUNCTUAL : aucun mercredi dans la plage → pas de conflit', async () => {
    ...
    mockFindMany.mockResolvedValue([makePrismaDecl({
      kind: 'UNAVAILABLE', recurKind: 'PUNCTUAL', dayOfWeek: null,
  ...
  it('nouveau PUNCTUAL vs RECURRING : couvre un mercredi → conflit', async () => {
    mockFindMany.mockResolvedValue([makePrismaDecl({ kind: 'UNAVAILABLE', recurKind: 'RECURRING', dayOfWeek: 3 })]);
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'PUNCTUAL' as const, slot: 'EVENING' as DaySlot, startDate: '2026-07-01', endDate: '2026-07-07', expiresAt: '2026-07-07T23:59:59Z' };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(1);
  });
  ...
  it('nouveau PUNCTUAL vs PUNCTUAL chevauchants → conflit', async () => {
    ...
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'PUNCTUAL' as const, slot: 'EVENING' as DaySlot, startDate: '2026-07-05', endDate: '2026-07-15', expiresAt: '2026-07-15T23:59:59Z' };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(1);
  });
  ...
  it('nouveau PUNCTUAL vs PUNCTUAL sans chevauchement → pas de conflit', async () => {
    ...
    const dto = { kind: 'AVAILABLE' as const, recurKind: 'PUNCTUAL' as const, slot: 'EVENING' as DaySlot, startDate: '2026-07-10', endDate: '2026-07-15', expiresAt: '2026-07-15T23:59:59Z' };
    expect(await service.findConflictsForCreate(USER_ID, dto)).toHaveLength(0);
  });
  ...
  it('excludeId → passe la clause NOT à Prisma', async () => {
    ...
    expect(mockFindMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ NOT: { id: 'skip-me' } }) }),
    );
  });
});
...
describe('AvailabilityService.create — conflict detection', () => {
  ...
  it('sans conflit → retourne { created: [declaration] }', async () => {
    mockFindMany.mockResolvedValue([]);
    const result = await service.create(USER_ID, baseDto) as { created: unknown[] };
  ...
  it('conflictResolution=overwrite → soft-delete les conflits puis crée', async () => {
    mockFindMany.mockResolvedValue([makePrismaDecl({ id: 'conflict-1', kind: 'UNAVAILABLE', recurKind: 'RECURRING', dayOfWeek: 3 })]);
    await service.create(USER_ID, { ...baseDto, conflictResolution: 'overwrite' });
    expect(mockUpdateMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ id: { in: ['conflict-1'] } }) }),
    );
    expect(mockCreate).toHaveBeenCalledTimes(1);
  });
  ...
  it('conflictResolution=keep + RECURRING new + PUNCTUAL conflit → crée des pièces', async () => {
    ...
    mockFindMany.mockResolvedValue([makePrismaDecl({
      id: 'conflict-1', kind: 'UNAVAILABLE', recurKind: 'PUNCTUAL', dayOfWeek: null,
      startDate: WED2, endDate: WED2,
    })]);
    const result = await service.create(USER_ID, { ...baseDto, startDate: '2026-06-24', conflictResolution: 'keep' }) as { created: unknown[] };
