 ...
 */
export function mapHommeDragonToPdfFields(dto, content) {
    ...
    for (let i = 0; i < MAX_HISTORIQUE_ROWS; i++) {
        ...
        fields.push({ field: `sc${i + 1}`, value: entry?.scenarioTitle ?? '', kind: 'text' }, { field: `date_sc_${i + 1}`, value: entry ? formatDateFr(entry.date) : '', kind: 'text' }, { field: `voy_sc_${i + 1}`, value: entry?.participants.join(', ') ?? '', kind: 'text' });
    }
    return fields;
}
