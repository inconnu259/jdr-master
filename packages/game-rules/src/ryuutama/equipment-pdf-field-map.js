...
function numericWeightOrZero(weight) {
    return isValidWeight(weight) ? weight : 0;
}
export function mapEquipmentToPdfFields(input) {
    const totalWeight = input.equipment.individual.reduce((sum, item) => sum + numericWeightOrZero(item.weight), 0) +
    ...
    for (let i = 0; i < OBJECT_SLOTS.length; i++) {
        ...
        fields.push({ field: objetField, value: item?.name ?? '', kind: 'text' }, { field: prixField, value: item?.price ?? '', kind: 'text' }, { field: encField, value: isValidWeight(item?.weight) ? String(item.weight) : '', kind: 'text' });
    }
    ...
    for (let i = 0; i < CONTENANT_SLOTS.length; i++) {
        ...
        fields.push({ field: objetField, value: item?.name ?? '', kind: 'text' }, { field: prixField, value: item?.price ?? '', kind: 'text' }, { field: encField, value: isValidWeight(item?.weight) ? String(item.weight) : '', kind: 'text' }, { field: effetsField, value: item?.effect ?? '', kind: 'text' });
    }
    ...
    for (let i = 0; i < ANIMAL_SLOTS.length; i++) {
        ...
        fields.push({ field: objetField, value: item?.name ?? '', kind: 'text' }, { field: prixField, value: item?.price ?? '', kind: 'text' }, { field: effetsField, value: item?.effect ?? '', kind: 'text' });
    }
    return fields;
}
