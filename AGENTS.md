...
- Ne pas déduire d'une CI verte que tout est vérifié : le `typecheck` de l'API s'appuie sur `tsconfig.build.json`, qui exclut `test/` et `**/*spec.ts` — les specs ne sont pas type-vérifiées (`tsconfig.check.json` les couvre mais remonte 101 erreurs de typage de mocks, chantier ouvert). CodeQL et `pnpm audit` tournent dans leurs propres workflows et **n'entrent pas dans la porte `ci-ok`** : ils informent, ils ne bloquent pas un merge.
