...
- Ne pas déduire d'une CI verte que tout est vérifié : `.github/workflows/ci.yml` n'exécute pas encore le lint du front, et ne type-vérifie ni les specs de l'API (`tsconfig.build.json` exclut `test/` et `**/*spec.ts`) ni la sécurité (aucun Semgrep, aucun `pnpm audit`). Les limites sont détaillées en commentaire en tête du workflow.
