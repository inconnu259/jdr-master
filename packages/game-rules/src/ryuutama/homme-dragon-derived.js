 ...
 */
export const HOMME_DRAGON_LEVEL_THRESHOLDS = [
...
export function levelForScenariosPasse(count) {
    let level = 1;
    for (const entry of HOMME_DRAGON_LEVEL_THRESHOLDS) {
        if (count >= entry.scenariosPasse)
            level = entry.level;
    }
    return level;
}
...
export function computeHommeDragonDerived(level) {
    if (level <= 2)
        return { PS: 3 };
    if (level <= 4)
        return { PS: 5 };
 ...
 */
export function pendingEveilLevels(currentLevel, appliedLevels) {
    const pending = [];
    for (let level = 2; level <= currentLevel; level++) {
        if (!appliedLevels.includes(level))
            pending.push(level);
    }
    return pending;
}
