...
describe('InviteLinksService', () => {
  ...
  it('create : MJ requis, token généré, expiration par défaut +7j', async () => {
    ...
    const link = (await service.create('p1', 'mj1', {})) as { token: string; expiresAt: Date };
  ...
  it('consumeLink : 404 si lien inconnu', async () => {
    ...
    await expect(service.consumeLink(tx as never, 'tok', 'u')).rejects.toBeInstanceOf(NotFoundException);
  });

  it('consumeLink : 403 si révoqué', async () => {
    const tx = makeTx({ id: 'l1', revoked: true, expiresAt: future(), maxUses: null, usesCount: 0, partieId: 'p1', partie: { mjId: 'mj1' } });
    await expect(service.consumeLink(tx as never, 'tok', 'u')).rejects.toBeInstanceOf(ForbiddenException);
  });

  it('consumeLink : 409 si l’utilisateur est le MJ', async () => {
    const tx = makeTx({ id: 'l1', revoked: false, expiresAt: future(), maxUses: null, usesCount: 0, partieId: 'p1', partie: { mjId: 'mj1' } });
    await expect(service.consumeLink(tx as never, 'tok', 'mj1')).rejects.toBeInstanceOf(ConflictException);
  });

  it('consumeLink : 409 si déjà membre', async () => {
    const tx = makeTx(
      { id: 'l1', revoked: false, expiresAt: future(), maxUses: null, usesCount: 0, partieId: 'p1', partie: { mjId: 'mj1' } },
      { userId: 'u', partieId: 'p1' },
    );
    await expect(service.consumeLink(tx as never, 'tok', 'u')).rejects.toBeInstanceOf(ConflictException);
  });

  it('consumeLink : 403 si le quota est épuisé (claim atomique = 0)', async () => {
    const tx = makeTx(
      { id: 'l1', revoked: false, expiresAt: future(), maxUses: 1, usesCount: 1, partieId: 'p1', partie: { mjId: 'mj1' } },
      null,
      0,
    );
    await expect(service.consumeLink(tx as never, 'tok', 'u')).rejects.toBeInstanceOf(ForbiddenException);
  ...
  it('consumeLink : crée le membership et incrémente quand tout est ok', async () => {
    const link = { id: 'l1', revoked: false, expiresAt: future(), maxUses: null, usesCount: 0, partieId: 'p1', partie: { mjId: 'mj1' } };
    ...
    expect(tx.membership.create).toHaveBeenCalledWith({ data: { userId: 'u', partieId: 'p1' } });
