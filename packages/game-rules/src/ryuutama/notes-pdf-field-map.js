 ...
 */
export function mapNotesToPdfFields(input) {
    const fields = [];
    for (let i = 0; i < MAX_NOTE_ROWS; i++) {
        const note = input.notes[i];
        fields.push({ field: `Note.${i}.0`, value: note ? formatDateFr(note.createdAt) : '', kind: 'text' }, { field: `Note.${i}.1`, value: note?.text ?? '', kind: 'text' });
    }
    return fields;
}
